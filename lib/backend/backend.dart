...
import 'schema/allowances_record.dart';
...
export 'schema/allowances_record.dart';
...
/// Functions to query AllowancesRecords (as a Stream and as a Future).
Future<int> queryAllowancesRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      AllowancesRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<AllowancesRecord>> queryAllowancesRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      AllowancesRecord.collection,
      AllowancesRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<AllowancesRecord>> queryAllowancesRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future maybeCreateUser(User user) async {
  ...
  if (userExists) {
    currentUserDocument = await UserListRecord.getDocumentOnce(userRecord);
    return;
  }
  ...
  currentUserDocument =
      UserListRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createUserListRecordData(email: email));
}
