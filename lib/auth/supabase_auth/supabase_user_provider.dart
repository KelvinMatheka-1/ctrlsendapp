...
class CtrlsendSupabaseUser extends BaseAuthUser {
  CtrlsendSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> ctrlsendSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CtrlsendSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
